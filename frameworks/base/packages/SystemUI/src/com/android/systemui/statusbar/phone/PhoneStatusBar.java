...
public class PhoneStatusBar extends BaseStatusBar implements DemoMode {
    ...
    // [ADD]  broadcast receiver for showing/hiding navigation bar or some buttons

    private static final int NAVBAR_DISABLE_BACK		= 0x01;
    private static final int NAVBAR_DISABLE_HOME		= 0x02;
    private static final int NAVBAR_DISABLE_RECENT		= 0x04;
    private static final int NAVBAR_DISABLE_STANDBY		= 0x08;

    private Object mNavbarMutex = new Object();
    
    private boolean mNavBarForceHidden = false;

    private BroadcastReceiver mNavBarReceiver = new BroadcastReceiver() {
		@Override
		public void onReceive(Context context, Intent intent) {
		
			synchronized (mNavbarMutex) {
				String action = intent.getAction();
				boolean show = intent.getBooleanExtra("show", true);
				if (!show) {
					Log.d("navbar", "disable navigation bar");
					//mNavigationBarView.setVisibility(View.GONE);
					mNavBarForceHidden = true;
					try {
					    removeBar();
					} catch (Exception e) {
					    Log.e("navbar", "disable failed");
					    e.printStackTrace();
					}
				} else {
					Log.d("navbar", "enable navigation bar");
					//mNavigationBarView.setVisibility(View.VISIBLE);
					mNavBarForceHidden = false;
					try {
					    addBarInside();
					} catch (Exception e) {
					    Log.e("navbar", "enable failed");
					    e.printStackTrace();
					}
				}
	
				// enable // disable buttons
				int disable = intent.getIntExtra("disable", 0);
				int disabledFlags = 0;
	
				if ((disable & NAVBAR_DISABLE_BACK) != 0)	disabledFlags |= View.STATUS_BAR_DISABLE_BACK;
				if ((disable & NAVBAR_DISABLE_HOME) != 0)	disabledFlags |= View.STATUS_BAR_DISABLE_HOME;
				if ((disable & NAVBAR_DISABLE_RECENT) != 0)	disabledFlags |= View.STATUS_BAR_DISABLE_RECENT;
	
				mNavigationBarView.getStandByButton().setVisibility(((disable & NAVBAR_DISABLE_STANDBY) == 0) ?
						View.VISIBLE : View.INVISIBLE);
	
				// enable/disable buttons
				mNavigationBarView.setDisabledFlags(disabledFlags, false /*force*/);
			}
		}
    };
	...
	// [ADD]  register broadcast receiver for showing/hiding navigation bar items
	intentfilter = new IntentFilter();
	intentfilter.addAction("adam.android.navbar.SHOW");
	context.registerReceiver(mNavBarReceiver, intentfilter);
    ...
    public void showSearchPanel() {
        ...
        if (mNavigationBarView != null) {
            ...
            //$_rbox_$_modify_$_chenxiao,add power & add/remove bar button
            if (mBarIsAdd)
	...
	mNavigationBarView.getRecentsButton().setOnTouchListener(mRecentsPreloadOnTouchListener);
        ...
        updateSearchPanel();
        //$_rbox_$_modify_$_chenxiao begin, add power & add/remove bar button
	...
	// [ADD] set touch listener
        mNavigationBarView.getStandByButton().setOnTouchListener(mStandByOnTouchListener);
    }
	...
	// [ADD] touch listener for standby button
	
	private View.OnTouchListener mStandByOnTouchListener = new View.OnTouchListener() {
		...
		public boolean onTouch(View v, MotionEvent event) {
			...
			if (action == MotionEvent.ACTION_DOWN) {
				...
				Intent intent = new Intent("adam.android.navbar.standby.CLICK");
				mContext.sendBroadcast(intent);
			}
			return false;
		}
	...
	private static final int READING_LIGHT_LEVEL_MAX = 5;
	private int mReadingLightLevel = 0;
	private View.OnTouchListener mReadingLightOnTouchListener = new View.OnTouchListener() {
	
		@Override
		public boolean onTouch(View v, MotionEvent event) {

			Log.d(TAG, "ReadingLight -> OnTouchListener()   action = " + (event.getAction() & MotionEvent.ACTION_MASK));

			int action = event.getAction() & MotionEvent.ACTION_MASK;
			if (action == MotionEvent.ACTION_DOWN) {

				switch (v.getId()) {
					case R.id.reading_light_down: {
						if (mReadingLightLevel <= 0)
							return false;

						--mReadingLightLevel;
						break;
					}
					case R.id.reading_light_up: {
						if (mReadingLightLevel >= READING_LIGHT_LEVEL_MAX)
							return false;
				...
				// safe check
				if (mReadingLightLevel < 0)
					mReadingLightLevel = 0;
				else if (mReadingLightLevel > READING_LIGHT_LEVEL_MAX)
					mReadingLightLevel = READING_LIGHT_LEVEL_MAX;
				
				int r = mReadingLightLevel * (255 / READING_LIGHT_LEVEL_MAX);

				Log.d(TAG, "reading light[0-255] = " + r);

				// set brightness
				setReadingLightBrightness(r);
			}
			return false;
		}
	};
	...
	static private void setReadingLightBrightness(int brightness) {
	
		BufferedWriter w = null;
		try {
			w = new BufferedWriter(new FileWriter("/sys/class/pwm/led_white/brightness"));
                	w.write(Integer.toString(brightness));
                } catch (Exception ex) {
                } finally {
            		try {
				if (w != null)
	...
	// [ADD]
	private Object mNavBarAddRemove = new Object();
	
	private boolean mBarIsAdd = true;
	private void addBarInside(){
	
		synchronized (mNavBarAddRemove) {

			// [ADD]
			if (mNavBarForceHidden)
				return;
